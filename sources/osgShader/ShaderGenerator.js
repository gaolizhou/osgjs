...
ShaderGenerator.prototype = {
    ...
    getActiveAttributeList: function ( state, list ) {
        ...
        for ( var j = 0, k = attributeArray.length; j < k; j++ ) {

            var attributeStack = attributeArray[ j ];
    ...
    getActiveAttributeListCache: function ( state ) {
        ...
        var hash = '';
        var attributeArray = state.lastAppliedAttribute;
    ...
    getActiveTextureAttributeList: function ( state, list ) {
        var hash = '';
        var attributeArrayList = state.textureAttributeArrayList;
        var i, l;

        for ( i = 0, l = attributeArrayList.length; i < l; i++ ) {
            var attributeArrayForUnit = attributeArrayList[ i ];

            if ( !attributeArrayForUnit ) continue;

            list[ i ] = [];

            for ( var j = 0, m = attributeArrayForUnit.length; j < m; j++ ) {

                var attributeStack = attributeArrayForUnit[ j ];
                if ( !attributeStack ) continue;
